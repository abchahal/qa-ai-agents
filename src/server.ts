...
server.registerTool(
  ...
  async ({ feature_description, use_input_folder = true }) => {
    try {
      ...
      console.error('\n╔═══════════════════════════════════════╗');
      console.error('║       QA AI Agent Pipeline            ║');
      console.error('╚═══════════════════════════════════════╝\n');

      console.error('[1/4] Agent 1 — Generating test scenarios...');
      const scenarios = await runFunctionalTester(finalInput);
      console.error(`[1/4] Done — ${scenarios.length} scenarios generated\n`);

      console.error('[2/4] Agent 2 — Assigning test pyramid layers...');
      const layerPlan = await runTestArchitect(scenarios);
      console.error(`[2/4] Done — ${layerPlan.length} scenarios assigned\n`);

      console.error('[3/4] Agent 3 — Writing Playwright test files...');
      const testResults = await runTestEngineer(layerPlan);
      console.error(`[3/4] Done — ${testResults.length} test files written\n`);

      console.error('[4/4] Agent 4 — Reviewing test quality...');
      const reviewReport = await runCodeReviewer(testResults);
      console.error(`[4/4] Done — Score: ${reviewReport.overallScore}/100\n`);
      ...
      const report = `# QA Pipeline Report
...
| ID | Type | Title |
|----|------|-------|
${scenarios.map(s => `| ${s.id} | ${s.type} | ${s.title} |`).join('\n')}
...
### Assignments
${layerPlan.map(lp => `- **${lp.scenarioId}** → \`${lp.layer}\` — ${lp.reason}`).join('\n')}
...
${testResults.length === 0
  ...
  : testResults.map(r => `- \`${r.file}\` — ${r.title}`).join('\n')
}
...
${reviewReport.issues && reviewReport.issues.length > 0
  ? reviewReport.issues.map(i => `- [**${i.severity.toUpperCase()}**] \`${i.file}\` — ${i.issue}`).join('\n')
  : '_No issues found._'
}

### Suggestions
${reviewReport.suggestions && reviewReport.suggestions.length > 0
  ? reviewReport.suggestions.map(s => `- ${s}`).join('\n')
  : '_No suggestions._'
}
...
`;
      ...
      fs.writeFileSync(path.join('output', 'pipeline_report.md'), report, 'utf8');

      const duration = ((Date.now() - pipelineStart) / 1000).toFixed(1);
      console.error(`\nPipeline complete in ${duration}s — report saved to output/pipeline_report.md\n`);
...
Report saved to: output/pipeline_report.md
      `.trim();
