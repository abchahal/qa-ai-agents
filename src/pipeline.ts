...
import { Scenario, LayerPlan, TestResult, ReviewReport } from "./types/types.js";
...
async function runPipeline(featureInput: string): Promise<PipelineResult> {
  ...
  if (fs.existsSync(path.join('output', 'tests'))) {
    ...
    fs.cpSync(path.join('output', 'tests'), path.join(archiveDir, 'tests'), { recursive: true });
    fs.cpSync(path.join('output', 'pages'), path.join(archiveDir, 'pages'), { recursive: true });
  ...
  layerSpinner.succeed(`Agent 2 — Assigned ${layerPlan.length} scenarios to layers`);
...
`;
  ...
  console.log(`  E2E tests written   : ${testResults.length}`);
